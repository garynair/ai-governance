#!/usr/bin/env python3
"""Regenerate the Governance Atlas reading-view HTML from README.md.
...
OUTPUT = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "dist" / "governance-atlas.html"
...
def wrap_sections(html: str) -> tuple[str, list[tuple[str, str, int]]]:
    start_idx = html.find('<h2 id="why-governance-matters">')
    if start_idx == -1:
        raise SystemExit("Could not find 'Why Governance Matters' section — is README.md structure unchanged?")
    ...
    while i < len(parts):
        ...
        content_clean = content_clean.replace("<ul>", '<ul class="entry-list">')
        content_clean = content_clean.replace("<li>", '<li class="entry">')
